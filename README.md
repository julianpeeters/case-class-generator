...
While Scala Macro Annotations are under development (and for projects stuck on scala 2.9), I'll use ASM and Artisinal-Pickle-Maker to generate bytecode for case classes, and a custom ClassLoader and Java reflection to instantiate it (can't use Scala's typeOf[], at least until we've got a type in a later step!). 
...
##Warnings: 
0) Designed to dynamically provide a type parameter for Salat and Salat-Avro. Access seems to be limited to Java reflection and parsing the Scala sig directly, i.e. access via Scala 2.10 reflection is not yet supported.

1) There is currently an issue that is limiting the classes to only a couple fields per class.
