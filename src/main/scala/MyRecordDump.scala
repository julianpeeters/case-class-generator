...
/*
  ...
  val fieldData: List[FieldData] = List(FieldData("x","string","Ljava/lang/String;", "Ljava/lang/String;",25,176), FieldData("y","int","I", "Ljava/lang/Object;",21,172), FieldData("z","boolean","Z", "Ljava/lang/Object;",21,172))//classData.classFields
   ...
   // val fieldData: List[FieldData] = List(FieldData("x","string","Ljava/lang/String;", "Ljava/lang/String;",25,176), FieldData("y","string","Ljava/lang/String;", "Ljava/lang/String;",25,176))
...
val str = new String(ByteCodecs.encode(mySig.bytes.map(_.toByte)))
println(str.toCharArray.length)
//println(stringSig.toCharArray.length)
//str.diff(stringSig).foreach(println)
//str.foreach(println)
...
println(str.length == reStr.length)
//str.diff(reStr).foreach(println)
...
//stringSig.foreach(println)
//println( stringSig.getBytes == str.getBytes)
//((str.getBytes).diff(stringSig.getBytes)).foreach(println)
println("str length: " + str.length)
println("stringSig: " + stringSig.length)
//val ssss = str.toCharArray.map(n => n.toByte)
//println("ssss: " + ssss.length)
val stringSigArray = Arrays.copyOf(stringSig.getBytes("UTF-8"), (ByteCodecs.decode(stringSig.getBytes("UTF-8")))-1)
val reString = new String(ByteCodecs.encode(stringSigArray))
println("reStringSig: " + reString.length)

println("stringSig last: " + stringSig.last)
println("reStringSig last: " + reString.last)
//reString.getBytes.diff(stringSig.getBytes).foreach(println)

println(stringSig == reString)
//stringSig.diff(reString).foreach(println)




val mySigArray = (Arrays.copyOf(str.getBytes("UTF-8"), ByteCodecs.decode(str.getBytes)))
println(stringSigArray == mySigArray)
//stringSigArray.foreach(println)
println("+______________++++++++++++++++++_______________________+++++++++++++++++++")
//mySigArray.foreach(println)
println("stringSigArray length: " + stringSigArray.length)
println("mySigArray length: " + mySigArray.length)

println("diff: ") 
//(stringSigArray.diff(mySigArray)).foreach(println)
...
//av0.visit("bytes", ByteCodecs.encode(mySig.bytes).mkString);
av0.visit("bytes", sig);
