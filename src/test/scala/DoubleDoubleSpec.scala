...
//but for now lets make it easy debug my Scala signature issue (chokes on > 3 fields even tho sig bytes are ok before encoding)
  val valueMembers: List[FieldSeed] = List(FieldSeed("a","Double"), FieldSeed("b","Double"))//, FieldSeed("d","Boolean"))
  val classData = ClassData("models", "MyRecord_DoubleDoubleSpec", valueMembers, FieldMatcher.getReturnTypes(valueMembers))
