...
//but for now lets make it easy debug my Scala signature issue (chokes on > 3 fields even tho sig bytes are ok before encoding)
  val valueMembers: List[FieldSeed] = List(FieldSeed("a","byte"))//, FieldSeed("b","int"))//, FieldSeed("d","boolean"))
  val classData = ClassData("models", "MyRecord_ByteSpec", valueMembers, FieldMatcher.getReturnTypes(valueMembers))
